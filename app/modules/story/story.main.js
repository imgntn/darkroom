...
var Story={
	...
	makeStoryElement:function(currentLocation){
		...
		// we need a global variable 
        	...
        	$.each(scrollText, function(i,elm){
        		$(story.storyEl).delay(delay*1E3).fadeOut();
        		$(story.storyEl).queue(function(){
        			$(story.storyEl).html(scrollText[i]);
        			$(story.storyEl).dequeue();
        		});

        		$(story.storyEl).fadeIn();
        		$(story.storyEl).queue(function(){
	...
	makeTheStory:function(){
		...
		this.storyEl.className="overlay_text";
		document.body.appendChild(this.storyEl);
